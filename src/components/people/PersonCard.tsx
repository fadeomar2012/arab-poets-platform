      ...
      {person.image ? (
        <div className="avatar avatar-image">
          <Image
            ...
            alt={localize(person.image.alt, locale)}
            fill
            sizes="72px"
          />
        </div>
        ...
        <div className="avatar" aria-hidden="true">
          {localize(person.initials, locale)}
        </div>
      )}
      <h3>{localize(person.name, locale)}</h3>
      <p>
        {localize(person.role, locale)} — {localize(person.country, locale)}
      </p>
      <Link className="text-link" href={`/${locale}/people/${person.slug}`}>
