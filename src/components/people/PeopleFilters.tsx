...
export function PeopleFilters({ people, locale }: { people: Person[]; locale: Locale }) {
  ...
  return <>
    <div className="filter-panel card filter-people"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={locale === "ar" ? "ابحث بالاسم" : "Search by name"} /><select value={role} onChange={(event) => setRole(event.target.value)}><option value="all">{locale === "ar" ? "كل الأدوار" : "All roles"}</option>{roles.map((item) => <option key={item}>{item}</option>)}</select><button className="button button-dark" onClick={() => { setQuery(""); setRole("all"); }}>{locale === "ar" ? "إعادة الضبط" : "Reset"}</button></div>
